...
Nesse sentido, é intuitivo obter a distribuição do máximo a partir da
...
sempre $F$ e/ou seus parâmetros são conhecidos, e estimá-los poderia
levar a erros significativos. Desse modo, estimar a distribuição de
$Y_M$ diretamente, desconsiderando a distribuição da população, é mais
adequado.
...
O algoritmo será usado para diferentes tamanhos de amostra (n = 10, n =
...
*distribuição de valor extremo generalizado*, e tem expressão da
acumulada dada por
...
Para confirmar que a Gumbel se aproxima melhor como distribuição
...
N <- 1E4
n <- 10
x10 <- replicate(N, max(rnorm(n)))
qqnorm(x10)

quantis_gumbel <- -log(-log((1:N)/(N+1)))
## Faz a mesma coisa que qqplot
plot(quantis_gumbel, sort(x10))
...
n <- 100
x100 <- replicate(N, max(rnorm(n)))
qqnorm(x100)
qqplot(quantis_gumbel,x100)
```

```{r simulacao3}

n <- 1000
x1000 <- replicate(N, max(rnorm(n)))
qqnorm(x1000)
qqplot(quantis_gumbel,x1000)
```

```{r simulacao4}
n <- 1E4
x10000 <- replicate(N, max(rnorm(n)))
qqnorm(x10000)
qqplot(quantis_gumbel,x10000)
```
